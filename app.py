...
st.markdown(f"""
    ...
    /* DEN HEADER KOMPLETT UNSICHTBAR MACHEN & PLATZ ENTFERNEN */
    ...
    /* Padding der App oben entfernen */
    .block-container {{
    ...
    /* Transparentes Glas-Design für die Karte */
    .main-card {{
        ...
        color: white; 
        text-align: center; 
        margin-top: 2vh; /* Karte rutscht weit nach oben */
        box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.5);
    }}
    
    .timer-display {{ 
        font-size: 80px; font-weight: 100; font-family: 'Courier New', monospace; 
        margin: 20px 0; color: #fff; text-shadow: 0 0 15px rgba(0,0,0,0.5);
    }}
...
if os.path.exists("sirene-da-monique.mp3"):
    try:
        ...
        st.error(f"Fehler beim Laden der Sirene: {e}")

# --- 3. PHASEN ---

if st.session_state.phase == "SETUP":
    st.title("🧘 ZenStretch")
    st.write("Voreinstellung: 20 Minuten Konzentration.")
    
    col1, col2, col3 = st.columns(3)
    with col1: hrs = st.number_input("Std", 0, 23, 0)
    with col2: mins = st.number_input("Min", 0, 59, 20) # 20 Min Default
    with col3: secs = st.number_input("Sek", 0, 59, 0)  # 0 Sek Default
    ...
    <div id="root" style="text-align: center; color: white; font-family: sans-serif;">
        <div id="countdown-area">
            ...
            <p style="letter-spacing: 2px; opacity: 0.7;">FOKUSSIERT BLEIBEN...</p>
        </div>

        <div id="exercise-area" style="display: none;">
            <h2 style="color: #ff4b4b; margin: 0; letter-spacing: 5px; text-shadow: 0 0 15px rgba(255,75,75,0.6);">🚨 ALARM 🚨</h2>
            ...
            <p id="status" style="margin-top: 10px; font-size: 18px; color: #ff4b4b; font-weight: bold;">STOPPE DIE SIRENE DURCH EINE VORBEUGE!</p>
        </div>
    </div>
    ...
    <script>
        ...
        async function startAlarmMode() {{
            ...
            alarm.play().catch(e => console.log("Audio gestoppt."));
            startCamera();
        }}
