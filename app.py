import streamlit as st
import streamlit.components.v1 as components
import time

# --- 1. KONFIGURATION & STYLING ---
st.set_page_config(page_title="ZenStretch Pro", layout="centered")

VIDEO_URL = "https://raw.githubusercontent.com/nschmitzyy/dehnweckerr/main/247740_medium.mp4"
POSTER_URL = "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&q=80&w=1000"

st.markdown(f"""
    <style>
    #bgVideo {{
        position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
        z-index: -1; object-fit: cover; filter: brightness(40%);
        background: url({POSTER_URL}) center/cover no-repeat;
    }}
    .stApp {{ background: transparent !important; }}
    .main-card {{
        background: rgba(0, 0, 0, 0.4);
        backdrop-filter: blur(20px);
        -webkit-backdrop-filter: blur(20px);
        border-radius: 30px; padding: 40px;
        border: 1px solid rgba(255, 255, 255, 0.1);
        color: white; text-align: center; margin-top: 5vh;
    }}
    .timer-display {{ 
        font-size: 100px !important; 
        font-weight: 100; 
        margin: 10px 0;
        font-family: monospace;
    }}
    header, footer {{ visibility: hidden !important; }}
    .stButton>button {{
        width: 100%; border-radius: 50px; background: white; color: black;
        font-weight: bold; padding: 12px; margin-top: 20px; border: none;
    }}
    </style>
    <video autoplay muted loop playsinline id="bgVideo" poster="{POSTER_URL}">
        <source src="{VIDEO_URL}" type="video/mp4">
    </video>
    """, unsafe_allow_html=True)

# --- 2. LOGIK ---
if 'phase' not in st.session_state:
    st.session_state.phase = "SETUP"

st.markdown('<div class="main-card">', unsafe_allow_html=True)

# PHASE 1: SETUP
if st.session_state.phase == "SETUP":
    st.title("🧘 ZenStretch")
    st.write("Wann möchtest du geweckt werden?")
    col1, col2 = st.columns(2)
    with col1:
        mins = st.number_input("Minuten", 0, 60, 0)
    with col2:
        secs = st.number_input("Sekunden", 0, 59, 10)
    
    if st.button("Timer starten"):
        st.session_state.total_seconds = (mins * 60) + secs
        st.session_state.phase = "COUNTDOWN"
        st.rerun()

# PHASE 2 & 3: COUNTDOWN & ALARM
else:
    timer_placeholder = st.empty()
    
    if st.session_state.phase == "COUNTDOWN":
        for i in range(st.session_state.total_seconds, 0, -1):
            timer_placeholder.markdown(f"<p class='timer-display'>{i}</p>", unsafe_allow_html=True)
            time.sleep(1)
        st.session_state.phase = "ALARM"
        st.rerun()

    if st.session_state.phase == "ALARM":
        timer_placeholder.markdown(f"<p class='timer-display' style='color:#ff4b4b;'>0</p>", unsafe_allow_html=True)
        st.audio("https://cdn.pixabay.com/audio/2022/03/15/audio_206684742d.mp3", autoplay=True)

        # Das Herzstück: HTML/JS mit Kamera-Berechtigung
        js_camera_code = """
        <div style="text-align: center; color: white; font-family: sans-serif;">
            <div style="position: relative; display: inline-block; border-radius: 20px; overflow: hidden; border: 2px solid white;">
                <video id="vid" style="width: 100%; max-width: 400px; transform: scaleX(-1); background: #222;" autoplay playsinline></video>
            </div>
            <p id="msg" style="margin-top: 10px;">Kamera wird aktiviert...</p>
            <div style="width: 100%; height: 8px; background: rgba(255,255,255,0.2); border-radius: 4px; overflow: hidden; margin-top: 10px;">
                <div id="bar" style="width: 0%; height: 100%; background: white; transition: 0.1s;"></div>
            </div>
        </div>

        <script type="module">
            import { Pose } from "https://cdn.jsdelivr.net/npm/@mediapipe/pose/pose.js";
            import { Camera } from "https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils/camera_utils.js";

            const video = document.getElementById('vid');
            const msg = document.getElementById('msg');
            const bar = document.getElementById('bar');
            let counter = 0;

            async function startApp() {
                try {
                    const pose = new Pose({locateFile: (f) => `https://cdn.jsdelivr.net/npm/@mediapipe/pose/${f}`});
                    pose.setOptions({ modelComplexity: 0, minDetectionConfidence: 0.5 });

                    pose.onResults(res => {
                        msg.innerText = "Bitte Pose einnehmen (Arme hoch!)";
                        if (res.poseLandmarks) {
                            const lm = res.poseLandmarks;
                            if (lm[15].y < lm[0].y && lm[16].y < lm[0].y) {
                                counter++;
                                bar.style.width = (counter / 1.5) + "%";
                                msg.innerText = "Haltung erkannt! Halten...";
                                if (counter >= 150) msg.innerText = "Übung beendet!";
                            }
                        }
                    });

                    const camera = new Camera(video, {
                        onFrame: async () => { await pose.send({image: video}); },
                        width: 480, height: 360
                    });
                    await camera.start();
                    msg.innerText = "Kamera aktiv!";
                } catch (err) {
                    msg.innerText = "Fehler: " + err.message;
                }
            }
            startApp();
        </script>
        """
        # HIER IST DIE WICHTIGE ÄNDERUNG: allow="camera"
        components.html(js_camera_code, height=500, scrolling=False)
        
    if st.button("Zurück zum Start"):
        st.session_state.phase = "SETUP"
        st.rerun()

st.markdown('</div>', unsafe_allow_html=True)
